...
description: >-
  End-to-end “skill studio” for Cursor and Claude Code: interviews non-technical
  users in plain language, drafts SKILL.md, runs side-by-side practice
  comparisons (with vs without the new instructions), collects structured
  feedback, iterates, tunes when the skill should attach, checks MCPs, and
  suggests external MCPs. Use when anyone wants to create, upgrade, or capture a
  repeatable AI workflow as a Cursor or Claude Code skill, or says skill
  builder, teach Cursor my process, recipe for the AI, playbooks, agent habits,
  or skill creator like Claude.
---

## Installing this skill (Claude Code and Cursor)

**Claude Code** loads each skill from a folder named after the skill that contains **`SKILL.md`**. Install this file as:

- **One repository:** `<project root>/.claude/skills/simple-skill-creator/SKILL.md`
- **Everywhere for your user (global):** `~/.claude/skills/simple-skill-creator/SKILL.md`  
  On Windows, use `%USERPROFILE%\.claude\skills\simple-skill-creator\SKILL.md`. If you use a custom Claude config directory, put the same path under that tree’s `skills/` folder (see [Claude Code `.claude` directory](https://code.claude.com/docs/en/claude-directory)).

**Cursor** uses the same `SKILL.md` filename under:

- **One repository:** `<project root>/.cursor/skills/simple-skill-creator/SKILL.md`
- **Optional user-wide:** `~/.cursor/skills/simple-skill-creator/SKILL.md`

The GitHub repo for this skill documents **`npx`**, **`curl`**, and clone-and-copy so the file lands in the right place for your editor.
...
2. **Location first** — Always ask **this project only** vs **all my projects** (Cursor: all Cursor projects; Claude Code: user/global skills) before writing files (see Step A).
...
Offer two choices in their words. Respect **which product they use** (Cursor, Claude Code, or both): each uses its own skills directory with the same `<skill-folder>/SKILL.md` shape.

| They pick | Cursor — you create files under | Claude Code — you create files under |
|-----------|----------------------------------|--------------------------------------|
| **This project only** | `<repo>/.cursor/skills/<skill-folder>/` | `<repo>/.claude/skills/<skill-folder>/` |
| **All my projects** (user / global) | `~/.cursor/skills/<skill-folder>/` | `~/.claude/skills/<skill-folder>/` |
